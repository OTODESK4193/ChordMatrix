...
namespace ChordMatrix {
        ...
        : audioProcessor(p), categoryModel(*this)
    {
        ...
        btnApply.onClick = [this] { applyPresetToProcessor(); if (onApplyPreset) onApplyPreset(); };
    ...
    void ProgressionBrowserComponent::loadPresets() {
        ...
        for (const auto& p : dict) {
            ...
            if (it != categories.end()) it->presets.push_back(p);
            else categories.push_back({ p.category, {p} });
        }
    }
    ...
    void ProgressionBrowserComponent::CategoryListModel::paintListBoxItem(int row, juce::Graphics& g, int w, int h, bool isSelected) {
        ...
        if (name.isNotEmpty()) g.drawText(name, 15, 0, w - 30, h, juce::Justification::centredLeft, true);
    }

    void ProgressionBrowserComponent::CategoryListModel::listBoxItemClicked(int row, const juce::MouseEvent&) {
        owner.selectedCategory = row; owner.selectedPreset = -1; owner.presetList.updateContent(); owner.repaint();
    }

    int ProgressionBrowserComponent::getNumRows() {
        if (selectedCategory >= 0 && selectedCategory < (int)categories.size()) return (int)categories[selectedCategory].presets.size();
        return 0;
    }
    ...
    void ProgressionBrowserComponent::paintListBoxItem(int row, juce::Graphics& g, int w, int h, bool isSelected) {
        ...
        if (itemText.isNotEmpty()) g.drawText(itemText, 15, 0, w - 30, h, juce::Justification::centredLeft, true);
    }

    void ProgressionBrowserComponent::listBoxItemClicked(int row, const juce::MouseEvent& e) {
        selectedPreset = row; repaint();
        if (e.getNumberOfClicks() == 2) { applyPresetToProcessor(); if (onApplyPreset) onApplyPreset(); }
    }
    ...
    void ProgressionBrowserComponent::applyPresetToProcessor() {
        ...
        for (int b = 0; b < preset.numBars; ++b) {
            ...
            for (int s = 0; s < stepsPerBar; ++s) {
                ...
                sData.inversion = 0; sData.shift = 0; sData.voicingMode = 0;
                for (int v = 0; v < 7; ++v) { sData.voices[v].isActive = false; sData.voices[v].octaveShift = 0; sData.voices[v].accidental = 0; }
        ...
        for (const auto& chord : preset.chords) {
            ...
            if (targetStep < ChordMatrix::TotalSteps) {
                ...
                sData.keyRoot = baseKey; sData.scaleType = baseScale;
                sData.chordDegree = chord.chordDegree; sData.voicingMode = chord.voicingMode;
                sData.gateLength = chord.lengthBeats * ppqPerBeat;

                sData.voices[0].isActive = (chord.accRoot != -128); sData.voices[0].accidental = (chord.accRoot == -128) ? 0 : chord.accRoot;
                sData.voices[1].isActive = (chord.acc3rd != -128); sData.voices[1].accidental = (chord.acc3rd == -128) ? 0 : chord.acc3rd;
                sData.voices[2].isActive = (chord.acc5th != -128); sData.voices[2].accidental = (chord.acc5th == -128) ? 0 : chord.acc5th;
                sData.voices[3].isActive = (chord.acc7th != -128); sData.voices[3].accidental = (chord.acc7th == -128) ? 0 : chord.acc7th;
    ...
    void ProgressionBrowserComponent::paint(juce::Graphics& g) {
        g.fillAll(juce::Colour(0xff1c1c1c)); g.setColour(juce::Colour(0xff333333)); g.drawRect(getLocalBounds(), 2);
        if (selectedCategory >= 0 && selectedCategory < (int)categories.size() && selectedPreset >= 0 && selectedPreset < (int)categories[selectedCategory].presets.size()) {
            const auto& preset = categories[selectedCategory].presets[selectedPreset];
            if (preset.previewText.isNotEmpty()) { g.setColour(juce::Colours::white); g.setFont(juce::Font(18.0f, juce::Font::bold)); g.drawText(preset.previewText, 20, getHeight() - 60, getWidth() - 300, 40, juce::Justification::centredLeft); }
        }
        else {
            g.setColour(juce::Colours::grey); g.setFont(16.0f); g.drawText("Select a progression to preview.", 20, getHeight() - 60, getWidth() - 300, 40, juce::Justification::centredLeft);
        }
    }

    MatrixGridComponent::MatrixGridComponent(ChordMatrixAudioProcessor& p) : audioProcessor(p), progressionBrowser(p)
    {
        progBtnBounds = juce::Rectangle<int>(leftMargin, 15, 110, 30); modulationBtnBounds = juce::Rectangle<int>(leftMargin + 120, 15, 110, 30);
        memoryBtnBounds = juce::Rectangle<int>(leftMargin + 240, 15, 80, 30); dragMidiBtnBounds = juce::Rectangle<int>(leftMargin + 330, 15, 100, 30);
        allClearBtnBounds = juce::Rectangle<int>(leftMargin + 440, 15, 100, 30);

        setupModulationPanel();
        addAndMakeVisible(progressionBrowser);
        progressionBrowser.onApplyPreset = [this]() { isProgressionMode = false; progressionBrowser.setVisible(false); if (onRepaintRequest) onRepaintRequest(); };
        progressionBrowser.onCancel = [this]() { isProgressionMode = false; progressionBrowser.setVisible(false); repaint(); };
    ...
    void MatrixGridComponent::setupModulationPanel() {
        addAndMakeVisible(modTargetBarMenu); addAndMakeVisible(modKeyMenu); addAndMakeVisible(modScaleMenu); addAndMakeVisible(modMethodMenu);
        addAndMakeVisible(btnModPreview); addAndMakeVisible(btnModApply); addAndMakeVisible(btnModCancel);
        ...
        for (int i = 0; i < modNames.size(); ++i) modMethodMenu.addItem(modNames[i], i + 1);

        modTargetBarMenu.setSelectedId(5, juce::dontSendNotification); modKeyMenu.setSelectedId(4, juce::dontSendNotification);
        modScaleMenu.setSelectedId(2, juce::dontSendNotification); modMethodMenu.setSelectedId(2, juce::dontSendNotification);

        auto setBtnStyle = [](juce::TextButton& btn, juce::Colour c) { btn.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff2a2a2a)); btn.setColour(juce::TextButton::textColourOffId, c); };
        setBtnStyle(btnModPreview, juce::Colours::cyan); setBtnStyle(btnModApply, juce::Colours::hotpink); setBtnStyle(btnModCancel, juce::Colours::grey);

        btnModPreview.onClick = [this] {
            int targetBar = modTargetBarMenu.getSelectedId() - 1; int targetKey = modKeyMenu.getSelectedId() - 1;
            int targetScale = modScaleMenu.getSelectedId() - 1; int methodIdx = modMethodMenu.getSelectedId() - 1;
            ...
            audioProcessor.isInternalPlaying = true; repaint();
            };

        btnModApply.onClick = [this] { audioProcessor.sequenceData = audioProcessor.previewSequenceData; audioProcessor.isPlayingModulationPreview.store(false); audioProcessor.isInternalPlaying = false; isModulationPanelOpen = false; resized(); if (onRepaintRequest) onRepaintRequest(); };
        btnModCancel.onClick = [this] { audioProcessor.isPlayingModulationPreview.store(false); audioProcessor.isInternalPlaying = false; isModulationPanelOpen = false; resized(); repaint(); };

        modTargetBarMenu.setVisible(false); modKeyMenu.setVisible(false); modScaleMenu.setVisible(false); modMethodMenu.setVisible(false);
        btnModPreview.setVisible(false); btnModApply.setVisible(false); btnModCancel.setVisible(false);
    }

    void MatrixGridComponent::setProgressionMode(bool isProg) { isProgressionMode = isProg; progressionBrowser.setVisible(isProgressionMode); repaint(); }
    ...
    int MatrixGridComponent::getEffectiveStep(int targetS) const {
        ...
        for (int prevS = targetS; prevS >= 0; --prevS) {
            ...
            // ★完全な無音・空ステップの排除（isActiveを絶対基準とする）
            bool hasNotes = false;
            for (int v = 0; v < 7; ++v) {
                if (sData.voices[v].isActive) { hasNotes = true; break; }
            }

            if (hasNotes && sData.gateLength > dist + 0.001f) { eff = prevS; break; }
        }
        return eff;
    }

    juce::Rectangle<float> MatrixGridComponent::getCellBounds(int s, int v, float stepW) const { return { leftMargin + static_cast<float>(s) * stepW, 155.0f + static_cast<float>(v) * cellHeight, stepW, cellHeight - 2.0f }; }
    juce::Rectangle<float> MatrixGridComponent::getStepHeaderBounds(int s, float stepW) const { return { leftMargin + static_cast<float>(s) * stepW, 155.0f - headerHeight - 35.0f, stepW, headerHeight - 5.0f }; }
    juce::Rectangle<float> MatrixGridComponent::getBarButtonBounds(int i) const { float w = seqTotalWidth / 8.0f; return { leftMargin + static_cast<float>(i % 8) * w, 555.0f + static_cast<float>(i / 8) * 35.0f, w - 8.0f, 30.0f }; }

    void MatrixGridComponent::resized() {
        if (isModulationPanelOpen) {
            int py = 665;
            modTargetBarMenu.setBounds(static_cast<int>(leftMargin) + 10, py, 85, 30); modKeyMenu.setBounds(static_cast<int>(leftMargin) + 100, py, 65, 30);
            modScaleMenu.setBounds(static_cast<int>(leftMargin) + 170, py, 130, 30); modMethodMenu.setBounds(static_cast<int>(leftMargin) + 305, py, 160, 30);
            btnModPreview.setBounds(static_cast<int>(leftMargin) + 475, py, 80, 30); btnModApply.setBounds(static_cast<int>(leftMargin) + 560, py, 80, 30); btnModCancel.setBounds(static_cast<int>(leftMargin) + 645, py, 80, 30);
        }
        bool showMod = isModulationPanelOpen && !isMemoryModeOpen && !isProgressionMode;
        modTargetBarMenu.setVisible(showMod); modKeyMenu.setVisible(showMod); modScaleMenu.setVisible(showMod); modMethodMenu.setVisible(showMod);
        btnModPreview.setVisible(showMod); btnModApply.setVisible(showMod); btnModCancel.setVisible(showMod);
    ...
    void MatrixGridComponent::paint(juce::Graphics& g) {
        const auto bg = juce::Colour(0xff1a1a1a); const auto activeColor = juce::Colour(0xffffa500); const auto panelBg = juce::Colour(0xff252525);
        const auto textLight = juce::Colours::white; const auto gridLine = juce::Colour(0xff333333);

        g.fillAll(bg);

        auto drawBtn = [&](int x, int y, int w, int h, const char* txt, bool active, juce::Colour c) {
            g.setColour(active ? c : juce::Colour(0xff3a3a3a)); g.fillRoundedRectangle(static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h), 4.0f);
            g.setColour(active ? juce::Colour(0xff1a1a1a) : textLight); g.setFont(12.0f); juce::String labelText(txt);
            if (labelText.isNotEmpty()) g.drawText(labelText, x, y, w, h, juce::Justification::centred);
            };
        ...
        if (isMemoryModeOpen) {
            g.setColour(panelBg); g.fillRoundedRectangle(leftMargin, 155.0f - headerHeight - 35.0f, seqTotalWidth, 7.0f * cellHeight + headerHeight + 65.0f, 8.0f);
            g.setColour(textLight); g.setFont(juce::Font(24.0f, juce::Font::bold)); g.drawText("MEMORY SLOTS", static_cast<int>(leftMargin), 130, static_cast<int>(seqTotalWidth), 30, juce::Justification::centred);
            float slotW = (seqTotalWidth - 120.0f) / 4.0f; float startX = leftMargin + 30.0f;
            for (int i = 0; i < 4; ++i) {
                juce::Rectangle<float> r(startX + i * (slotW + 20.0f), 220.0f, slotW, 150.0f); bool isUsed = audioProcessor.isSlotUsed[i];
                g.setColour(isUsed ? activeColor.withAlpha(0.8f) : juce::Colour(0xff3a3a3a)); g.fillRoundedRectangle(r, 8.0f);
                g.setColour(isUsed ? bg : juce::Colours::grey); g.setFont(juce::Font(24.0f, juce::Font::bold));
                juce::String memTitle = "MEMORY " + juce::String(i + 1); if (memTitle.isNotEmpty()) g.drawText(memTitle, r, juce::Justification::centred);
                g.setFont(juce::Font(12.0f, juce::Font::plain)); juce::Rectangle<float> textR = r.translated(0, 45.0f);
                if (isUsed) { g.drawText("L-Click: LOAD", textR.removeFromTop(15.0f), juce::Justification::centred); g.drawText("Shift+Click: OVRWRT", textR.removeFromTop(15.0f), juce::Justification::centred); g.drawText("R-Click: CLEAR", textR.removeFromTop(15.0f), juce::Justification::centred); }
                else { g.drawText("L-Click: SAVE", textR.removeFromTop(15.0f), juce::Justification::centred); }
            }
            return;
        }

        g.setFont(12.0f); g.setColour(juce::Colours::grey); g.drawText("INV", 0, 155 - 30, static_cast<int>(leftMargin) - 10, 25, juce::Justification::centredRight);
        static const char* const vNames[] = { "6/13th", "4/11th", "2/9th", "7th", "5th", "3rd", "Root" };
        for (int i = 0; i < 7; ++i) { juce::String vn(vNames[i]); if (vn.isNotEmpty()) g.drawText(vn, 0, 155 + static_cast<int>(static_cast<float>(i) * cellHeight), static_cast<int>(leftMargin) - 10, static_cast<int>(cellHeight), juce::Justification::centredRight); }
        g.setColour(juce::Colours::indianred); g.drawText("DEL", 0, 155 + static_cast<int>(7.0f * cellHeight), static_cast<int>(leftMargin) - 10, 30, juce::Justification::centredRight);

        int editBar = (int)*audioProcessor.apvts.getRawParameterValue("editBar");
        int stepsPerBar = getStepsPerBar(); float ppqPerStep = getPpqPerStep(); float stepW = seqTotalWidth / static_cast<float>(stepsPerBar);
        ...
        for (int s = 0; s < stepsPerBar; ) {
            int actS = (editBar * stepsPerBar) + s; int effS = getEffectiveStep(actS); auto& effStep = activeSeqData[effS];

            int runLength = 1;
            for (int nextS = s + 1; nextS < stepsPerBar; ++nextS) { if (getEffectiveStep((editBar * stepsPerBar) + nextS) == effS) runLength++; else break; }

            float startX = leftMargin + static_cast<float>(s) * stepW; float runW = static_cast<float>(runLength) * stepW;
            bool isSelected = false; for (int i = 0; i < runLength; ++i) { if (getEffectiveStep((editBar * stepsPerBar) + s + i) == getEffectiveStep(selectedStep)) isSelected = true; }

            juce::Rectangle<float> rHeader = getStepHeaderBounds(s, stepW).withWidth(runW);
            g.setColour(isSelected ? activeColor.withAlpha(0.3f) : panelBg); g.fillRoundedRectangle(rHeader.reduced(1.0f), 4.0f);
            ...
            if (recognizedName.contains("\n")) {
                juce::String part1 = recognizedName.upToFirstOccurrenceOf("\n", false, false).trim(); juce::String part2 = recognizedName.fromFirstOccurrenceOf("\n", false, false).replaceCharacter('(', ' ').replaceCharacter(')', ' ').trim();
                if (part1.isNotEmpty()) { g.setFont(juce::Font(13.0f, juce::Font::bold)); g.setColour(juce::Colour(0xffffa500)); g.drawFittedText(part1, textRect.removeFromTop(textRect.getHeight() / 2), juce::Justification::centredBottom, 1, 0.1f); }
                if (part2.isNotEmpty()) { g.setFont(juce::Font(12.0f, juce::Font::plain)); g.setColour(juce::Colours::white); g.drawFittedText(part2, textRect, juce::Justification::centredTop, 1, 0.1f); }
            }
            else {
                if (recognizedName.isNotEmpty()) { g.setFont(juce::Font(13.0f, juce::Font::bold)); g.setColour(juce::Colour(0xffffa500)); g.drawFittedText(recognizedName, textRect, juce::Justification::centred, 2, 0.1f); }
            }

            juce::Rectangle<float> rInv(startX, 155.0f - 30.0f, runW, 25.0f);
            g.setColour(juce::Colour(0xff2a2a2a)); g.fillRoundedRectangle(rInv.reduced(1.0f), 4.0f); g.setFont(12.0f);

            if (VoicingEngine::isAutoPattern(effStep.voicingMode)) { g.setColour(juce::Colours::grey.withAlpha(0.5f)); g.drawText("INV: --", rInv, juce::Justification::centred); }
            else { g.setColour(juce::Colours::lightgreen); juce::String invStr = "INV: " + juce::String(effStep.inversion); if (invStr.isNotEmpty()) g.drawText(invStr, rInv, juce::Justification::centred); }

            juce::Rectangle<float> rDel(startX, 155.0f + (7.0f * cellHeight), runW, 30.0f);
            g.setColour(juce::Colour(0xff331111)); g.fillRoundedRectangle(rDel.reduced(1.0f), 4.0f); g.setColour(juce::Colours::red.withAlpha(0.7f)); g.drawText("X", rDel, juce::Justification::centred);

            s += runLength;
        }

        for (int s = 0; s <= stepsPerBar; ++s) { g.setColour((s % stepsPerBeat == 0) ? juce::Colour(0xff555555) : gridLine); g.drawLine(leftMargin + static_cast<float>(s) * stepW, 155.0f - 5.0f, leftMargin + static_cast<float>(s) * stepW, 155.0f + (8.0f * cellHeight), (s % stepsPerBeat == 0) ? 2.0f : 1.0f); }
        ...
        if (audioProcessor.isPlaying && globalStep >= 0 && totalStepsInLoop > 0 && ((globalStep % totalStepsInLoop) / stepsPerBar) == editBar) {
            g.setColour(juce::Colours::white.withAlpha(0.15f)); g.fillRect(leftMargin + static_cast<float>((globalStep % totalStepsInLoop) % stepsPerBar) * stepW, 155.0f - 5.0f, stepW, 8.0f * cellHeight + 10.0f);
        }

        for (int s = 0; s < stepsPerBar; ++s) { for (int v = 0; v < 7; ++v) { g.setColour(juce::Colour(0xff121212)); g.fillRect(getCellBounds(s, v, stepW).reduced(1.0f, 1.0f)); } }

        for (int s = 0; s < stepsPerBar; ++s) {
            int actS = (editBar * stepsPerBar) + s; auto& effStep = activeSeqData[actS];

            // ★空ステップの完全無効化（描画スキップ）
            bool hasNotes = false;
            for (int v = 0; v < 7; ++v) { if (effStep.voices[v].isActive) { hasNotes = true; break; } }
            ...
            if (VoicingEngine::isAutoPattern(effStep.voicingMode)) {
                ...
                for (int i = 0; i < maxV; ++i) {
                    ...
                    if (!effStep.voices[i].isActive) {
                        g.setColour(activeColor.withAlpha(0.2f)); g.drawRect(cell.reduced(1.0f, 1.0f), 1.0f);
                    }
                    else {
                        g.setColour(activeColor); g.fillRect(cell.reduced(1.0f, 1.0f));
                        if (audioProcessor.isPlaying && globalStep >= 0 && totalStepsInLoop > 0 && actS == (globalStep % totalStepsInLoop)) { g.setColour(textLight); g.drawRect(cell.reduced(1.0f, 1.0f), 2.0f); }
                        int p = tempPitches[i]; juce::String noteName = MusicTheory::getNoteName(p) + juce::String((p / 12) - 1);
                        if (noteName.isNotEmpty()) { g.setColour(bg); g.setFont(juce::Font(14.0f, juce::Font::bold)); g.drawFittedText(noteName, cell.reduced(2.0f).toNearestInt(), juce::Justification::centred, 1, 0.1f); }
                    }
                }
            }
            else {
                for (int v = 0; v < 7; ++v) {
                    int voiceIdx = 6 - v; auto& voice = effStep.voices[voiceIdx];
                    if (voice.isActive) {
                        auto cell = getCellBounds(s, v, stepW).withWidth(juce::jmin(desiredWidth, maxWidth));
                        g.setColour(activeColor); g.fillRect(cell.reduced(1.0f, 1.0f));
                        if (audioProcessor.isPlaying && globalStep >= 0 && totalStepsInLoop > 0 && actS == (globalStep % totalStepsInLoop)) { g.setColour(textLight); g.drawRect(cell.reduced(1.0f, 1.0f), 2.0f); }
                        ...
                        juce::String label; if (totalOct != 0) label << (totalOct > 0 ? "+" : "") << totalOct;
                        if (voice.accidental > 0) { for (int i = 0; i < voice.accidental; ++i) { if (label.isNotEmpty()) label << " "; label << "#"; } }
                        else if (voice.accidental < 0 && voice.accidental > -128) { for (int i = 0; i < std::abs((int)voice.accidental); ++i) { if (label.isNotEmpty()) label << " "; label << "b"; } }
                        if (label.isNotEmpty()) { g.setColour(bg); g.setFont(juce::Font(14.0f, juce::Font::bold)); g.drawFittedText(label, cell.reduced(2.0f).toNearestInt(), juce::Justification::centred, 1, 0.1f); }
                    }
                }
            }
        }
        ...
        for (int i = 0; i < 16; ++i) {
            auto r = getBarButtonBounds(i); bool inLoop = (i < std::array<int, 5>{1, 4, 8, 12, 16}[loopBarsIndex]);
            g.setColour(i == editBar ? activeColor : (inLoop ? panelBg : juce::Colour(0xff121212))); g.fillRoundedRectangle(r, 4.0f);
            g.setColour(i == editBar ? bg : (inLoop ? textLight : juce::Colours::grey)); g.setFont(juce::Font(16.0f, juce::Font::bold));
            juce::String barText = "BAR " + juce::String(i + 1); if (barText.isNotEmpty()) g.drawText(barText, r, juce::Justification::centred);
        }
        ...
        if (isModulationPanelOpen) {
            ...
            if (modText.isNotEmpty()) g.drawFittedText(modText, static_cast<int>(leftMargin) + 10, 640, 350, 20, juce::Justification::centredLeft, 1, 0.5f);
        }
    }
    ...
    void MatrixGridComponent::mouseDown(const juce::MouseEvent& e) {
            ...
            juce::NativeMessageBox::showAsync(juce::MessageBoxOptions().withTitle("All Clear").withMessage("Clear ALL sequences and initialize?").withButton("Yes").withButton("No"),
                [safeThis](int result) {
                    if (safeThis != nullptr && result == 0) {
                        for (int s = 0; s < ChordMatrix::TotalSteps; ++s) { safeThis->audioProcessor.sequenceData[s] = {}; safeThis->audioProcessor.previewSequenceData[s] = {}; }
        ...
        float localX = static_cast<float>(e.x) - leftMargin; if (localX < 0) return;
        ...
        if (e.y >= 155.0f - 30.0f && e.y < 155.0f - 5.0f) {
            if (int localStep = static_cast<int>(localX / stepW); localStep < stepsPerBar) {
                ...
                if (activeNotesCount > 0) {
                    ...
                    std::array<int, 7> vps = { 0 }; int count = VoicingEngine::getVoicedPitches(audioProcessor.sequenceData[effS], vps);
        ...
        if (e.y >= 155.0f + 7.0f * cellHeight && e.y < 155.0f + 7.0f * cellHeight + 30.0f) {
            if (e.mods.isLeftButtonDown()) {
                if (int localStep = static_cast<int>(localX / stepW); localStep < stepsPerBar) {
                    int effS = getEffectiveStep(editBar * stepsPerBar + localStep); audioProcessor.sequenceData[effS] = {};
                    if (onStepSelected) onStepSelected(selectedStep = effS); if (onRepaintRequest) onRepaintRequest();
                }
            }
            return;
        }
        ...
        if (e.y >= 155.0f && e.y < 155.0f + 7.0f * cellHeight) {
            for (int s = 0; s < stepsPerBar; ++s) {
                int actS = editBar * stepsPerBar + s; auto& effStep = audioProcessor.sequenceData[actS];
                    ...
                    if (isCellActive) {
                        if (cell.contains(e.position)) {
                            if (e.mods.isRightButtonDown()) {
                                effStep.voices[voiceIdx].isActive = false; effStep.voices[voiceIdx].octaveShift = 0; effStep.voices[voiceIdx].accidental = 0;
                                bool anyActive = false; for (int i = 0; i < 7; ++i) if (effStep.voices[i].isActive) anyActive = true;
                            ...
                            if (e.mods.isLeftButtonDown()) {
                                if (e.position.x > cell.getRight() - 10.0f) { isDraggingGate = true; dragStep = actS; dragStartGate = safeGate; dragStartX = static_cast<float>(e.position.x); return; }
                                else {
                                    ...
                                    for (int iv = voiceIdx - 1; iv >= 0; --iv) if (effStep.voices[iv].isActive) { isLowestActive = false; break; }
                        ...
                        if (VoicingEngine::isAutoPattern(effStep.voicingMode)) {
                            ...
                            if (voiceIdx < currentMaxV) {
                                bool wasEmpty = true; for (int i = 0; i < 7; ++i) if (effStep.voices[i].isActive) wasEmpty = false;
                                if (wasEmpty) { for (int i = 0; i < currentMaxV; ++i) effStep.voices[i].isActive = true; }
                                else { effStep.voices[voiceIdx].isActive = true; }
                            ...
                            for (int prevS = actS - 1; prevS >= 0; --prevS) {
                                if (audioProcessor.sequenceData[prevS].voices[voiceIdx].isActive && audioProcessor.sequenceData[prevS].gateLength > static_cast<float>(actS - prevS) * getPpqPerStep() + 0.001f) { isCovered = true; break; }
                            }
                            if (!isCovered) {
    ...
    void MatrixGridComponent::mouseMove(const juce::MouseEvent& e) {
        if (isProgressionMode || isModulationPanelOpen || isMemoryModeOpen || audioProcessor.isPlayingModulationPreview.load()) { setMouseCursor(juce::MouseCursor::NormalCursor); return; }
        int editBar = (int)*audioProcessor.apvts.getRawParameterValue("editBar");
        int stepsPerBar = getStepsPerBar(); float stepW = seqTotalWidth / static_cast<float>(stepsPerBar);
        ...
        if (float localX = static_cast<float>(e.x) - leftMargin; localX >= 0 && e.y >= 155.0f && e.y < 155.0f + 7.0f * cellHeight) {
            for (int s = 0; s < stepsPerBar; ++s) {
                ...
                int maxV = 0;
                if (VoicingEngine::isAutoPattern(effStep.voicingMode)) { std::array<int, 7> dummy; maxV = VoicingEngine::getPatternBPitches(effStep, dummy); }
                ...
                for (int v = 0; v < 7; ++v) {
                    ...
                    if (isCellActive) {
                        ...
                        float maxWidth = seqTotalWidth - static_cast<float>(s) * stepW;
                        auto cell = getCellBounds(s, v, stepW).withWidth(juce::jmin(desiredWidth, maxWidth));
                        if (cell.contains(e.position) && e.position.x > cell.getRight() - 10.0f) { hoveringEdge = true; break; }
                    }
                }
            }
        }
    ...
    void MatrixGridComponent::mouseDrag(const juce::MouseEvent& e) {
        int stepsPerBar = getStepsPerBar(); float ppqPerStep = getPpqPerStep();
        if (!isDraggingMidi && dragMidiBtnBounds.contains(e.getMouseDownPosition()) && e.mouseWasDraggedSinceMouseDown()) {
            isDraggingMidi = true; int loopIdx = juce::jlimit(0, 4, (int)*audioProcessor.apvts.getRawParameterValue("loopBars"));
        ...
        if (!isDraggingMidi && e.mouseWasDraggedSinceMouseDown()) {
            for (int i = 0; i < 16; ++i) if (getBarButtonBounds(i).contains(e.getMouseDownPosition().toFloat())) {
                isDraggingMidi = true; ChordMatrix::MidiExport::exportAndDrag(audioProcessor.sequenceData, i, 16, stepsPerBar, ppqPerStep, this); return;
            }
        }
        ...
        if (isDraggingGate && dragStep >= 0) {
            float deltaX = static_cast<float>(e.position.x) - dragStartX; float maxGate = 16.0f; int localS = dragStep % stepsPerBar;
            ...
            for (int nextS = localS + 1; nextS < stepsPerBar; ++nextS) {
                ...
                if (targetFilled) { maxGate = static_cast<float>(nextS - localS) * ppqPerStep; break; }
            }
            ...
            if (int deltaSteps = std::round((static_cast<float>(e.position.x) - dragStartX) / stepW); deltaSteps != 0) {
                ...
                if (targetStep != dragStep) {
                    ...
                    if (targetEmpty) {
                        audioProcessor.sequenceData[targetStep] = audioProcessor.sequenceData[dragStep]; audioProcessor.sequenceData[dragStep] = {};
            ...
            for (int s = 0; s < stepsPerBar; ++s) {
                ...
                for (int v = 0; v < 7; ++v) {
                    ...
                    if (cell.contains(e.position) && isCellActive) {
                        ...
                        if (VoicingEngine::isAutoPattern(effStep.voicingMode)) voice.accidental = juce::jlimit<int8_t>(-24, 24, voice.accidental + (wheel.deltaY > 0 ? 1 : -1));
                        else if (e.mods.isCtrlDown() || e.mods.isCommandDown()) { if (voiceIdx != 0) voice.accidental = juce::jlimit<int8_t>(-2, 2, voice.accidental + (wheel.deltaY > 0 ? 1 : -1)); }
