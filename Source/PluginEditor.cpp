...
void ChordMatrixAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    g.drawText("ChordMatrix", 25, 0, 300, 65, juce::Justification::left);

    // Transport Buttons
    auto drawBtn = [&](int x, juce::String txt, bool active, juce::Colour c) {
        g.setColour(active ? c : juce::Colours::darkgrey.darker());
        g.fillRoundedRectangle(x, 15, 75, 35, 5.0f);
        g.setColour(juce::Colours::white); g.setFont(14.0f);
        g.drawText(txt, x, 15, 75, 35, juce::Justification::centred);
        };

    drawBtn(350, "SYNC", audioProcessor.isSyncEnabled, accentOrange);
    drawBtn(440, "PLAY", audioProcessor.isInternalPlaying, juce::Colours::green.withAlpha(0.8f));
    drawBtn(530, "STOP", !audioProcessor.isInternalPlaying && !audioProcessor.isSyncEnabled, juce::Colours::red.withAlpha(0.8f));

    // BPM Area
    g.setColour(juce::Colours::black); g.fillRoundedRectangle(630, 15, 140, 35, 5.0f);
    g.setColour(juce::Colours::white); g.setFont(juce::Font(18.0f, juce::Font::bold));
    g.drawText(juce::String(audioProcessor.displayBPM, 1) + " BPM", 630, 15, 140, 35, juce::Justification::centred);

    int editBar = (int)*audioProcessor.apvts.getRawParameterValue("editBar");
    int numBars = (int)*audioProcessor.apvts.getRawParameterValue("numBars");
    ...
    float stepW = (getWidth() - 140.0f) / 16.0f;
    for (int s = 0; s < 16; ++s) {
        int actS = (editBar * 16) + s;
        auto& beat = audioProcessor.beatSettings[actS / 4];
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        bool inL = (i < numBars);
        g.setColour(i == editBar ? juce::Colours::white : (inL ? juce::Colours::grey : juce::Colours::black));
...
{
    if (juce::Rectangle<int>(350, 15, 75, 35).contains(e.getPosition())) audioProcessor.isSyncEnabled = !audioProcessor.isSyncEnabled;
    if (juce::Rectangle<int>(440, 15, 75, 35).contains(e.getPosition())) { audioProcessor.isInternalPlaying = true; audioProcessor.isSyncEnabled = false; }
    if (juce::Rectangle<int>(530, 15, 75, 35).contains(e.getPosition())) { audioProcessor.isInternalPlaying = false; audioProcessor.isSyncEnabled = false; audioProcessor.internalPPQ = 0.0; }
    ...
    for (int i = 0; i < 16; ++i) if (getBarButtonBounds(i).contains(e.position)) audioProcessor.apvts.getParameter("editBar")->setValueNotifyingHost(i / 15.0f);
}

void ChordMatrixAudioProcessorEditor::mouseDrag(const juce::MouseEvent& e) {
    if (juce::Rectangle<int>(630, 15, 140, 35).contains(e.getMouseDownPosition())) {
...
juce::Rectangle<float> ChordMatrixAudioProcessorEditor::getCellBounds(int s, int v) { return { 100.0f + s * ((getWidth() - 140.0f) / 16.0f), 200.0f + v * 55.0f, (getWidth() - 140.0f) / 16.0f, 55.0f }; }
juce::Rectangle<float> ChordMatrixAudioProcessorEditor::getBeatLaneBounds(int b, int y) { return { 100.0f + b * (((getWidth() - 140.0f) / 16.0f) * 4), (float)y, ((getWidth() - 140.0f) / 16.0f) * 4, 45.0f }; }
juce::Rectangle<float> ChordMatrixAudioProcessorEditor::getBarButtonBounds(int i) { float w = (getWidth() - 100.0f) / 8.0f; return { 50.0f + (i % 8) * w, getHeight() - 120.0f + (i / 8) * 55.0f, w - 8.0f, 48.0f }; }
